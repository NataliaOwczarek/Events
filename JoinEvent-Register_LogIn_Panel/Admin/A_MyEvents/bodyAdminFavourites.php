...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        if ($eventResult->num_rows > 0) {
            ...
            echo "<a href='../A_Event/AdminEventDetails.php?id={$eventRow['EventId']}' class='btn btn-secondary'>Szczegóły wydarzenia</a>";
            echo "<button id='removeBtn{$eventId}' class='btn btn-favorite btn-remove-favorite' style='margin-top:0px;' data-event-id='{$eventRow['EventId']}'>";

            if (in_array($eventRow['EventId'], array_values(getFavouriteEventsbyUser($userId, $connection)))) {
    ...
    echo "<h4 class='alert-heading'>Brak ulubionych wydarzeń!</h4>";
    echo "<p>Nie masz jeszcze żadnych ulubionych wydarzeń. Dodaj je teraz!</p>";
    echo "<a href='../A_Home/AdminHome.php' class='btn btn-primary' style='background-color: #444; color: #fff; border: none;'>Przejdź do strony głównej</a>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
    echo "</div>";
...
document.addEventListener('DOMContentLoaded', function() {
    document.querySelectorAll('.btn-remove-favorite').forEach(button => {
        button.addEventListener('click', async function(event) {
            ...
            try {
                const response = await fetch(`../../Admin/A_Home/AdmintoggleFavourite.php?eventId=${eventId}`);
                        ...
                        <div class='row justify-content-center'>
                            <div class='col-md-8 text-center'>
                                <div class='alert alert-secondary' role='alert'>
                                    ...
                                    <a href='../A_Home/AdminHome.php' class='btn btn-primary' style='background-color: #444; color: #fff; border: none;'>Przejdź do strony głównej</a>
                                </div>
                            </div>
                        </div>
                    `;
