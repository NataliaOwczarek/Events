<div style="margin-top: 70px; margin-bottom: 70px;" class="container mt-8">
    ...
    <form method="post" action="<?php echo $_SERVER['PHP_SELF']; ?>" class="mb-3 p-2 rounded border bg-transparent" style="max-width: 300px;">
        ...
        <button type="submit" name="filterEvents" class="btn btn-secondary btn-sm">Filtruj: Wydarzenia, na które się zapisałeś</button>
    ...
    // Kod obsługujący przycisk "Cofnij filtrowanie"
    if (isset($_POST['resetFilter'])) {
        // Domyślne zapytanie, które pobiera wszystkie wydarzenia
        $sql = "SELECT * FROM Events_ListOfEvents WHERE EventDateTime > '$today' ORDER BY EventDateTime";
    } else {
        // Kod obsługujący przycisk "Filtruj"
...
async function toggleFavorite(button, eventId) {
    try {
        var response = await fetch(`AdmintoggleFavourite.php?eventId=${eventId}`);
...
async function toggleSingUpForEvent(button, eventId) {
    try {
        var response = await fetch(`AdmintoggleSingUpForEvent.php?eventId=${eventId}`);
        var result = await response.text();
