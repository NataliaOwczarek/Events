...
<body>
    ...
    <div class="container mt-5">
        <div class="row">
            ...
            if ($_SERVER["REQUEST_METHOD"] == "POST") {
                ...
                if ($stmt->execute()) {
                    echo "<script>window.location.href = 'UserEventDetails.php?id=" . $eventId . "';</script>";
                    exit;
                } else {
            ...
            $sql = "SELECT Events_ListOfEvents.*, Events_Users.* 
            ...
            if ($result && $result->num_rows > 0) {
                ...
                if ($eventDateTime >= $currentDateTime) {
                    ...
                    if ($eventDateTime < $currentDateTime) {
                        echo "<script>document.getElementById('editButton').style.display = 'none';</script>";
                    }
                ...
                if ($row['UserId'] == $userId && $eventDateTime >= $currentDateTime) {
    ...
    <script>
        ...
        try {
            var response = await fetch(`../U_Home/toggleFavourite.php?eventId=${eventId}`);
    ...
    async function toggleSingUpForEvent(button, eventId) {
        try {
            var response = await fetch(`../U_Home/toggleSingUpForEvent.php?eventId=${eventId}`);
            var result = await response.text();
    ...
    async function removeEvent(button, eventId) {
        ...
        if (confirmation) {
            try {
                ...
                if (result === 'removed') {
                    window.location.href = '../U_Home/UserHome.php';
                } else {
            ...
            window.location.href = `UserEventDetails.php?id=${eventId}`;
        }
    }
    </script>
